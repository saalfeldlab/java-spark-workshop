...
public class OverlappingMatricesToStrip {
	...
	public static class Options {
		...
		private final Boolean view = false;
	...
	public static FloatProcessor run(final Options o) {
...
//        {
		...
		final FloatProcessor result = new FloatProcessor(2 * range + 1, stop - start);

		for (int z = start; z < stop; z += step) {
			final int lower = Math.max(z - overlap, start);
			final int upper = Math.min(z + overlap + step, stop);
			System.out.println(z + " " + lower + " " + upper);

			final FloatProcessor m = (FloatProcessor) new ImagePlus(String.format(pattern, lower, upper)).getProcessor();
